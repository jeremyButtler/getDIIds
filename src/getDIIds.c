...
main(
   ...
   *   - output/input variables (one tempory)
   ...
   schar mvRnaBl = def_pMVRna_inputGetDIIds;
   schar vRnaBl = def_pVRna_inputGetDIIds;
   schar partBl = def_partSeq_inputGetDIIds;
   schar diffBl = def_noAgree_inputGetDIIds;

   schar keepBl = 0; /*holds if keep di/mv/v RNA read*/
   ...
   { /*Loop: find primers in sequences*/
      ...
      *   - check if is di/mvRNA and print
      ...
      { /*If: I found at least on primer*/
         ...
         keepBl |=
            ((!!(diFlagSC & def_mvFound_fluST)) &mvRnaBl);
         ...
         { /*Else: I have a expected segment length*/
            percLenDiffF = (float) mapLenUL;
            percLenDiffF /=
